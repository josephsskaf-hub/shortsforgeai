...
/**
 * Map platform name to Runway pixel-based ratio.
...
export async function generateScenes(prompt: string): Promise<string[]> {
    const userPrompt = `You break a Short-form video idea into 4 vivid, cinematic shot descriptions for an AI text-to-video model (RunwayML Gen-4.5).
...
export async function startRunwayTask(
    ...
    console.log(`[runway] sending to /text_to_video — model=${payload.model} ratio=${payload.ratio} duration=${payload.duration}`)
