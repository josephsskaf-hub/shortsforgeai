...
- "cinematic": an EPIC LANDSCAPE or AERIAL shot only (sweeping vista, drone reveal, vast environment). 8 seconds. May carry a short external narration line ("voiceover"). USE SPARINGLY: at most 1-2 "cinematic" scenes per video, and ONLY when the story truly needs an epic wide — every other non-dialogue scene is "support". Write its prompt with the SAME styleSheet look as everything else.
- "support": a simpler establishing/detail b-roll shot (ambient sound only, nobody speaks). 5 or 10 seconds. May carry a "voiceover" narration line.
...
Target total duration: ${Math.max(45, Math.min(60, Math.round(durationSeconds || 60)))} seconds.`
  ...
  for (let i = 0; i < Math.min(rawScenes.length, 7); i++) {
    ...
    prompt = `${prompt} Cinematography (match exactly): ${styleSheet}.${NO_TEXT_SUFFIX}`
