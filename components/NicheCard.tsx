...
export default function NicheCard({
  ...
  return (
    <div
      ...
      style={{
        background: selected ? 'rgba(16, 185, 129,.13)' : 'rgba(11,17,32,0.6)',
        backdropFilter: 'blur(16px) saturate(140%)',
        WebkitBackdropFilter: 'blur(16px) saturate(140%)',
        border: selected ? '1.5px solid rgba(16, 185, 129,.65)' : '1px solid rgba(255,255,255,0.08)',
        boxShadow: selected ? '0 0 0 1px rgba(16, 185, 129,.25) inset, 0 8px 36px rgba(16, 185, 129,.22)' : 'none',
      }}
      onMouseEnter={(e) => {
        if (selected) return
        ...
        el.style.boxShadow = '0 12px 40px rgba(16, 185, 129,.18)'
      }}
      onMouseLeave={(e) => {
        if (selected) return
        const el = e.currentTarget as HTMLElement
    ...
    >
      ...
      {selected && (
        <div
          ...
          style={{ background: 'linear-gradient(135deg, rgba(16, 185, 129,.1), rgba(5, 150, 105,.06) 70%)', zIndex: 0 }}
        />
      )}
      ...
      {!selected && (
        <div
          ...
          style={{ background: 'linear-gradient(135deg, rgba(16, 185, 129,.08), transparent 70%)' }}
        />
      )}
      ...
      <div className="flex items-start justify-between mb-3 relative z-10">
        <div
          ...
          style={{
            ...
            border: selected ? '1px solid rgba(16, 185, 129,.4)' : '1px solid rgba(16, 185, 129,.18)',
          }}
        >
          {emoji}
        </div>
      ...
      <div className="flex flex-wrap gap-1.5 mb-4 relative z-10">
        {tags.map((tag) => (
          <span
            ...
            style={{
              background: selected ? 'rgba(16, 185, 129,.15)' : 'rgba(16, 185, 129,.09)',
              border: selected ? '1px solid rgba(16, 185, 129,.28)' : '1px solid rgba(16, 185, 129,.14)',
