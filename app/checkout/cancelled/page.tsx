      ...
      <div style={{ width: '100%', maxWidth: 560, background: 'rgba(11,17,32,0.85)', border: '1px solid var(--border)', borderRadius: 22, padding: 'clamp(24px, 5vw, 36px)', boxShadow: '0 16px 60px rgba(0,0,0,.5)' }}>
        ...
        <div style={{ marginTop: 22, background: 'linear-gradient(135deg, rgba(37,99,235,.10), rgba(124,58,237,.06))', border: '1px solid rgba(37,99,235,.30)', borderRadius: 16, padding: 18 }}>
          ...
          <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }}>
            <a href={STRIPE_LINKS.basic} onClick={() => { trackEvent('basic_checkout_clicked'); trackCheckoutClick('basic') }} style={{ display: 'block', textAlign: 'center', textDecoration: 'none', padding: '12px 14px', borderRadius: 12, fontSize: '0.88rem', fontWeight: 900, color: '#fff', background: 'linear-gradient(135deg, #2563EB, #1D4ED8)' }}>Basic — $4.90/mo →</a>
            <a href={STRIPE_LINKS.pro} onClick={() => { trackEvent('pro_checkout_clicked'); trackCheckoutClick('pro') }} style={{ display: 'block', textAlign: 'center', textDecoration: 'none', padding: '12px 14px', borderRadius: 12, fontSize: '0.88rem', fontWeight: 900, color: '#fff', background: 'linear-gradient(135deg, #7c3aed, #a855f7)' }}>Pro — $9.90/mo →</a>
          </div>
        </div>
