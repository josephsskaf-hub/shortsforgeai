    ...
    <main style={{ minHeight: '100vh', background: 'var(--bg)', color: 'var(--text)', fontFamily: 'Inter, system-ui, sans-serif', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '32px 20px' }}>
      <div style={{ width: '100%', maxWidth: 560, background: 'rgba(11,17,32,0.85)', border: '1px solid var(--border)', borderRadius: 22, padding: 'clamp(24px, 5vw, 36px)', boxShadow: '0 16px 60px rgba(0,0,0,.5)' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ marginTop: 10, fontSize: '0.88rem', color: '#6ee7b7', fontWeight: 700 }}>Redirecting to pricing in {countdown}…</p>
        </div>
        <div style={{ marginTop: 22, background: 'linear-gradient(135deg, rgba(5,150,105,.10), rgba(5,150,105,.06))', border: '1px solid rgba(5,150,105,.30)', borderRadius: 16, padding: 18 }}>
          ...
          <div style={{ display: 'grid', gap: 10, gridTemplateColumns: 'repeat(2, minmax(0, 1fr))' }}>
            <a href={STRIPE_LINKS.basic} onClick={() => { trackEvent('basic_checkout_clicked'); trackCheckoutClick('basic') }} style={{ display: 'block', textAlign: 'center', textDecoration: 'none', padding: '12px 14px', borderRadius: 12, fontSize: '0.88rem', fontWeight: 900, color: '#fff', background: 'linear-gradient(135deg, #059669, #047857)' }}>Creator — $24.90/mo →</a>
            <a href={STRIPE_LINKS.pro} onClick={() => { trackEvent('pro_checkout_clicked'); trackCheckoutClick('pro') }} style={{ display: 'block', textAlign: 'center', textDecoration: 'none', padding: '12px 14px', borderRadius: 12, fontSize: '0.88rem', fontWeight: 900, color: '#fff', background: 'linear-gradient(135deg, #059669, #10b981)' }}>Studio — $37.90/mo →</a>
          </div>
        </div>
        <div style={{ marginTop: 18, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12, fontSize: '0.85rem' }}>
          <Link href="/pricing" style={{ color: '#6ee7b7', textDecoration: 'none', fontWeight: 700 }}>← Go back to pricing</Link>
