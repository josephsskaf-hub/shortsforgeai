...
export default function GenerateClient() {
  ...
  // #402 — is the user on the Studio plan? Gates the Cinematic AI (Kling) engine.
    ...
    async function fetchCredits() {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          // #402 — Studio gate for the Cinematic AI (Kling) engine.
...
function ModeSelector({
  ...
  const aiFeatures = ['Every scene generated by AI', 'Great-quality AI visuals', 'Fast render']
  ...
  return (
    <div className="mt-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {/* Push #403 — Fast Mode re-enabled as the Starter-tier engine (stock,
            now with per-scene relevance-gated Pixabay matching). */}
        {true && (
        <button
          type="button"
        ...
        <button
          type="button"
          onClick={() => { setMode('cinematic_ai'); setAiEngine('seedance') }}
          className="rounded-xl p-4 text-left"
          style={{
        ...
        >
          <div className="flex items-center gap-2 mb-2.5">
            ...
            <div className="ml-auto flex items-center gap-1.5">
              {/* #384 — "1 free" while the account still has its free AI trial. */}
              {freeAiUsed === false && (credits ?? 0) < 30 ? (
                <span
                  className="text-xs font-bold px-2 py-0.5 rounded-full"
                  style={{ background: 'rgba(16,185,129,.18)', color: '#34d399', border: '1px solid rgba(16,185,129,.35)' }}
                >
                  1 free · watermark
                </span>
              ) : (
                <span
                  className="text-xs font-bold px-2 py-0.5 rounded-full"
                  style={{ background: 'rgba(245,158,11,.18)', color: '#fcd34d', border: '1px solid rgba(245,158,11,.3)' }}
                >
                  30 credits
                </span>
              )}
