...
export const metadata: Metadata = {
  ...
  title: 'Affiliate Program — Earn 40% Recurring | ShortsForgeAI',
  description:
    'Promote ShortsForgeAI and earn 40% recurring commission, forever, with a 60-day cookie. Free account included. The AI tool that turns one idea into a finished faceless Short in 60s.',
  alternates: { canonical: 'https://www.shortsforgeai.com/partners' },
  openGraph: {
    title: 'Earn 40% recurring promoting ShortsForgeAI',
...
const APPLY = 'mailto:hello@shortsforgeai.com?subject=Affiliate%20application%20-%20ShortsForgeAI&body=Hi!%20I%27d%20like%20to%20join%20the%20affiliate%20program.%0A%0AMy%20channel%2Faudience%3A%20%0ANiche%3A%20%0AAudience%20size%3A%20'
...
export default function PartnersPage() {
  const faq = [
    ...
    { q: 'What do I promote?', a: 'ShortsForgeAI turns one idea into a finished faceless Short (script + AI voice + footage + captions) in about 60 seconds. It basically demos itself on camera — perfect for tutorials, reviews, and "I made this in 60s" content.' },
    ...
    <main style={{ minHeight: '100vh', background: '#0A0A0B', color: '#F1F5F9', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      ...
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '28px 18px 64px' }}>
        ...
        <section style={{ marginTop: 36, textAlign: 'center' }}>
          <div style={{ display: 'inline-block', fontSize: '0.72rem', fontWeight: 800, letterSpacing: '0.05em', textTransform: 'uppercase', color: '#22D3EE', background: 'rgba(34,211,238,0.1)', borderRadius: 999, padding: '6px 14px' }}>Affiliate program</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
            {[
              ...
              <div key={r.n} style={{ ...CARD, borderRadius: 14, padding: 18, textAlign: 'center' }}>
                ...
                <div style={{ color: '#94A3B8', fontSize: '0.82rem', margin: '2px 0 8px' }}>{r.d}</div>
                <div style={{ color: '#22D3EE', fontWeight: 800, fontSize: '0.95rem' }}>{r.e}</div>
              </div>
            ))}
          </div>
        ...
        <section style={{ marginTop: 44, textAlign: 'center', ...CARD, borderRadius: 18, padding: '28px 20px' }}>
          ...
          <a href={APPLY} style={{ display: 'inline-block', background: '#22D3EE', color: '#0A0A0B', fontWeight: 900, padding: '14px 30px', borderRadius: 12, textDecoration: 'none', fontSize: '1.02rem' }}>Apply now →</a>
        </section>
      </div>
    </main>
  )
